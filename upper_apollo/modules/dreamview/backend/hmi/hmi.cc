...
namespace
{
  ...
  int8_t parseState(const std::string& module, const std::string& command)
  {
    if (module == "AutoDrive")
    {
      if (command == "start")
...
void HMI::RegisterMessageHandlers() {
  ...
  AdapterManager::AddResCurrentStateCallback([this](const ::std_msgs::UInt8 &msg) {
    HMIWorker::instance()->ResCurrentStateCallback(msg);
  });
  ...
  websocket_->RegisterMessageHandler(
      ...
      [this](const Json &json, WebSocketHandler::Connection *conn) {
        ...
        if (JsonUtil::GetStringFromJson(json, "module", &module) &&
            JsonUtil::GetStringFromJson(json, "command", &command)) {
              const auto state = parseState(module, command);
              if (state >= 0)
              {
                ::std_msgs::UInt8 msg;
                msg.data = static_cast<uint8_t>(state);
                AdapterManager::PublishReqStateTransition(msg);
              }
        } else {
          AERROR << "Truncated module command.";
        }
