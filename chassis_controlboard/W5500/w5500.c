...
uint8_t tcp_constatus = 0;//add by hopehu
uint8_t Ir_status = 0;
/**
...
*@return	None
*/
uint8_t socket_buf_init( uint8 * tx_size, uint8 * rx_size  )
{
    int16 i;
    int16 ssum=0,rsum=0;
...
uint8_t get_constatus(uint8 * addr)
{
    static uint8_t close_status = 0;
    wiz_read_buf(PHYCFGR, addr, 1);

    if((!(*addr&0x01))&&(close_status == 0))
    {
        close_status = 1;
        close(SOCK_TCPS);
        tcp_connectflag = 0;
