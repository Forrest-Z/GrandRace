...
constexpr char kControlModuleName[] = "navigation_control";
constexpr char kRecordModuleName[] = "record_navi_bag";
constexpr char kChassisModuleName[] = "chassis";
constexpr char kChassisActiveSafetyModuleName[] = "chassis_activeSafety";
constexpr char kGuardianModuleName[] = "guardian";
...
void HMIWorker::RunModeCommand(const std::string &command_name) {
  ...
  if (command_name == "start" || command_name == "stop") {
    const bool use_navigation_mode = (current_mode == kNavigationModeName);
    // Run the command on all live modules.
    for (const auto &module : mode_conf.live_modules()) {
      // When using navigation mode, do not run module record_navi_bag
      // to avoid unnecessary CPU and disk waste.
      if (use_navigation_mode 
        && command_name == "start"
        && (module == kRecordModuleName || module == kControlModuleName 
            || module == kChassisModuleName || module == kChassisActiveSafetyModuleName)) 
      {
        continue;
      }
...
ModuleStatus* HMIWorker::GetModuleStatus(const std::string &module_name) {
  return &LookupOrInsert(status_.mutable_system_status()->mutable_modules(), module_name, {});
}

void HMIWorker::ResCurrentStateCallback(const ::std_msgs::UInt8& msg)
{
  const auto state = msg.data;
  const auto autoDriveState = 6u;
  const auto command = state == autoDriveState ? "start" : "stop";

  bool guardian_running = false;
  {
      RLock rlock(status_mutex_);
      auto module = GetModuleStatus(kGuardianModuleName);
      guardian_running = module ? module->process_status().running() : false;
  }
  if (guardian_running)
  {
    RunModuleCommand(kChassisActiveSafetyModuleName, command);
  }
  else
  {
    RunModuleCommand(kChassisModuleName, command);
  }
  
  RunModuleCommand(kControlModuleName, command);
}
