...
void LinSendCmd(ControlCommand *control_command) {
  // 
  LinControlCommand lin_control_command;
  lin_control_command.set_linear_velocity(control_command->linear_velocity()); //control_command->linear_velocity
  lin_control_command.set_angular_velocity(control_command->angular_velocity()); //control_command->angular_velocity
  AdapterManager::FillLinControlCommandHeader("lin_control", &lin_control_command);

  AdapterManager::PublishLinControlCommand(lin_control_command);
}

void Control::SendCmd(ControlCommand *control_command) {
  LinSendCmd(control_command);
  // set header
